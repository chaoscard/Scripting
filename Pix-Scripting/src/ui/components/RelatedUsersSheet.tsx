...
import { userRelated, recommendedUsers } from "../../api/pixiv"
...
export function RelatedUsersSheet(props: {
  ...
  const loadRelated = async () => {
    ...
    setError(null)
    try {
      let page = await session.call((token) => userRelated(seedUserID, token))
      if (!page.items || page.items.length === 0) {
        page = await session.call((token) => recommendedUsers(token))
      }
